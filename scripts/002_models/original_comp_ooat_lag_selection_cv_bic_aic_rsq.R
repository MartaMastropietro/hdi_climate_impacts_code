...
# cross validation for lag number, one extr var at the time , with without tp 
# lets check each variable either alone, tp, tp + all lags in other variables. 
...
out_variables<-c("gr_gnipc", "gr_eys", "gr_leb")
model_types<-c("lags_tp_n_mix_control", "lags_tp_n_8_control",  "lags_extr_alone", "lags_all_controls", "lags_all_controls_big")
mod_types<-c("mod_tm", "mod_rr", "mod_extr")
model_types_simple<-c("bhm_tp_control", "bhm_extr_alone", "fd_tp_control", "fd_extr_alone")
...
extr_formula_gnipc<- paste0(c(hw_formula, 
                        wd_formula, 
                        pext_formula),
                      collapse = "+")
...
extr_formula_leb<- paste0(c(hw_formula, 
                        rx_formula,
                        tvar_formula, 
                        wd_formula),
                      collapse = "+")
...
extr_formula_eys<- paste0(c(wd_formula, 
                        pext_formula,
                        rx_formula, 
                        tvar_formula),
                      collapse = "+")

extr_formula_complete<-list(
  "gr_gnipc"=extr_formula_gnipc,
  "gr_leb"=extr_formula_leb,
  "gr_eys"=extr_formula_eys
)
...
extr_formula_complete_big<-list(
  "gr_gnipc"=extr_formula_gnipc,
  "gr_leb"=extr_formula_leb,
  "gr_eys"=extr_formula_eys
)
...
remove_var_from_formula <- function(f_str, cl_var) {
  # Build pattern to match full terms containing cl_var (with optional leading '+')
  pattern <- paste0("(\\+)?[^\\+]*", cl_var, "[^\\+]*")
  # Remove the matching terms
  cleaned <- gsub(pattern, "", f_str)
  # Clean up multiple consecutive '+' signs
  cleaned <- gsub("\\++", "+", cleaned)
  # Remove '+' at start or end, if any
  cleaned <- gsub("^\\+|\\+$", "", cleaned)
  ...
  for (o in out_variables){
    ...
    for (cl_var in m_modns_cont[-c(1:2)]){
      ...
      if(o=="gr_eys"){
        ...
        # extr controls 
        extr_form_control<- extr_formula_complete[[o]]
        # Remove those terms
        extr_form_control <-remove_var_from_formula(extr_form_control,paste0("diff_",cl_var) ) 
        
        
        extr_big_form_control<- extr_formula_complete_big[[o]]
        # Remove those terms
        extr_big_form_control <-remove_var_from_formula(extr_big_form_control,paste0("diff_",cl_var) ) 
        ...
        r=paste0(mean_cl_formula_mix , "+", ext_formula)
        f= as.formula(paste( o, "~", r, "|" ,i ))
        m = fixest::feols(f, data , panel.id = pan_id)
        ...
        ### tp + extremes mod extremes + all other extremes controls
        model_type<-"lags_all_controls"
        mod_type<-"mod_extr"
        N_temp<-N_temp_fix_mix
        N_rain<-N_rain_fix_mix
        r=paste0(mean_cl_formula_mix , "+", ext_formula, "+", extr_form_control )
        f= as.formula(paste( o, "~", r, "|" ,i ))
        m = fixest::feols(f, data , panel.id = pan_id)
        ...
        ### tp + extremes mod extremes + all other extremes controls
        model_type<-"lags_all_controls_big"
        mod_type<-"mod_extr"
        N_temp<-N_temp_fix_mix
        N_rain<-N_rain_fix_mix
        r=paste0(mean_cl_formula_mix , "+", ext_formula, "+", extr_big_form_control )
        f= as.formula(paste( o, "~", r, "|" ,i ))
        m = fixest::feols(f, data , panel.id = pan_id)
        ...
        N_temp<-N_temp_fix_mix
        N_rain<-N_rain_fix_mix
        r=paste0(mean_cl_formula_mix , "+", ext_formula_tm)
        f= as.formula(paste( o, "~", r, "|" ,i ))
        m = fixest::feols(f, data , panel.id = pan_id)
        ...
        ### tp + extremes mod tm + all other extremes controls
        model_type<-"lags_all_controls"
        mod_type<-"mod_tm"
        N_temp<-N_temp_fix_mix
        N_rain<-N_rain_fix_mix
        r=paste0(mean_cl_formula_mix , "+", ext_formula_tm, "+", extr_form_control )
        f= as.formula(paste( o, "~", r, "|" ,i ))
        m = fixest::feols(f, data , panel.id = pan_id)
        ...
        N_temp<-N_temp_fix_mix
        N_rain<-N_rain_fix_mix
        r=paste0(mean_cl_formula_mix , "+", ext_formula_tm, "+", extr_big_form_control )
        f= as.formula(paste( o, "~", r, "|" ,i ))
        m = fixest::feols(f, data , panel.id = pan_id)
        ...
        N_temp<-N_temp_fix_mix
        N_rain<-N_rain_fix_mix
        r=paste0(mean_cl_formula_mix , "+", ext_formula_rr)
        f= as.formula(paste( o, "~", r, "|" ,i ))
        m = fixest::feols(f, data , panel.id = pan_id)
        ...
        ### tp + extremes mod rr + all other extremes controls
        model_type<-"lags_all_controls"
        mod_type<-"mod_rr"
        N_temp<-N_temp_fix_mix
        N_rain<-N_rain_fix_mix
        r=paste0(mean_cl_formula_mix , "+", ext_formula_rr, "+", extr_form_control )
        f= as.formula(paste( o, "~", r, "|" ,i ))
        m = fixest::feols(f, data , panel.id = pan_id)
        ...
        N_temp<-N_temp_fix_mix
        N_rain<-N_rain_fix_mix
        r=paste0(mean_cl_formula_mix , "+", ext_formula_rr, "+", extr_big_form_control )
        f= as.formula(paste( o, "~", r, "|" ,i ))
        m = fixest::feols(f, data , panel.id = pan_id)
...
for (o in out_variables){
  ...
  arrange<-ggpubr::ggarrange(g1,g3,g4,g5, common.legend = TRUE, nrow=1, ncol=5)
  ggsave(file.path(out_dir_cv,paste0(o,"_new_lag_select_oaat.png")), arrange, width = 35, height = 7)
}
...
for (o in out_variables){
  for (cl in unique(results$cl_var)){
    ...
    g1<-ggplot(res, aes(n_lags_EXTR, loocv, color=model_type))+geom_point(aes(shape=mod_type))+theme_bw()+labs(shape="moderating variable", color="model")
    #g2<-ggplot(res, aes(n_lags_EXTR, l3ocv, color=model_type))+geom_point(aes(shape=mod_type))+theme_bw()+labs(shape="moderating variable", color="model")
    g3<-ggplot(res, aes(n_lags_EXTR, wr2, color=model_type))  +geom_point(aes(shape=mod_type))+theme_bw()+labs(shape="moderating variable", color="model")
    g4<-ggplot(res, aes(n_lags_EXTR, aic, color=model_type))  +geom_point(aes(shape=mod_type))+theme_bw()+labs(shape="moderating variable", color="model")
    g5<-ggplot(res, aes(n_lags_EXTR, bic, color=model_type))  +geom_point(aes(shape=mod_type))+theme_bw()+labs(shape="moderating variable", color="model")
    
    arrange<-ggpubr::ggarrange(g1,g3,g4,g5, common.legend = TRUE, nrow=1, ncol=4)%>%ggpubr::annotate_figure( top = ggpubr::text_grob(paste0(o, "_", cl)))
    ggsave(file.path(out_dir_cv,paste0(o, "_",cl,"_new_lag_select_oaat.png")), arrange, width = 18, height = 4)
...
for (o in out_variables){
  for (cl in unique(results$cl_var)){
    ...
    res<-results%>%filter(out_var==o, cl_var==cl,model_type %in% c("lags_all_controls", "lags_all_controls_big") )
    ...
    arrange<-ggpubr::ggarrange(g1,g3,g4,g5, common.legend = TRUE, nrow=1, ncol=4)%>%ggpubr::annotate_figure( top = ggpubr::text_grob(paste0(o, "_", cl)))
    ggsave(file.path(out_dir_cv,paste0(o, "_",cl,"_no_simple_new_lag_select_oaat.png")), arrange, width = 18, height = 4)
    
  }
}


# one for out var with relevant vars only, correct moderating variables only 
out_dir_cv_final<-file.path(out_dir_cv, "final")
if(!dir.exists(out_dir_cv_final)){dir.create(out_dir_cv_final)}

results<-results%>%filter(n_lags_EXTR<9)
  
### hw - mean tm, wd - mean rr, pext - mean tm
o<-"gr_gnipc"
ovar<-"GNIPC"

cl="HW"
res<-results%>%filter(out_var==o, cl_var==cl, mod_type== "mod_tm", model_type %in% c("lags_all_controls", "lags_all_controls_big", "lags_tp_n_mix_control") )
res$mod_type<-"TM"
g1<-ggplot(res, aes(n_lags_EXTR, loocv, color=model_type))+geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))
#g2<-ggplot(res, aes(n_lags_EXTR, l3ocv, color=model_type))+geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))
g3<-ggplot(res, aes(n_lags_EXTR, wr2, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))
g4<-ggplot(res, aes(n_lags_EXTR, aic, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))
g5<-ggplot(res, aes(n_lags_EXTR, bic, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))

arrange<-ggpubr::ggarrange(g1,g3,g4,g5, common.legend = TRUE, legend="right", nrow=1, ncol=4)%>%ggpubr::annotate_figure( top = ggpubr::text_grob(paste0(ovar, " - ", cl)))
ggsave(file.path(out_dir_cv_final,paste0(o, "_",cl,"_final_lag_select_oaat.png")), arrange, width = 20, height = 4)


cl="WD"
res<-results%>%filter(out_var==o, cl_var==cl, mod_type== "mod_rr", model_type %in% c("lags_all_controls", "lags_all_controls_big", "lags_tp_n_mix_control") )
res$mod_type<-"RR"
g1<-ggplot(res, aes(n_lags_EXTR, loocv, color=model_type))+geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))
#g2<-ggplot(res, aes(n_lags_EXTR, l3ocv, color=model_type))+geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))
g3<-ggplot(res, aes(n_lags_EXTR, wr2, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))
...
### hw - mean hw, wd - mean tm, tvar - mean tvar, rx - mean tm
o<-"gr_leb"
ovar<-"LEB"
...
res<-results%>%filter(out_var==o, cl_var==cl, mod_type== "mod_tm", model_type %in% c("lags_all_controls", "lags_all_controls_big", "lags_tp_n_mix_control") )
res$mod_type<-"TM"
g1<-ggplot(res, aes(n_lags_EXTR, loocv, color=model_type))+geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
#g2<-ggplot(res, aes(n_lags_EXTR, l3ocv, color=model_type))+geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
g3<-ggplot(res, aes(n_lags_EXTR, wr2, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
g4<-ggplot(res, aes(n_lags_EXTR, aic, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
g5<-ggplot(res, aes(n_lags_EXTR, bic, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
...
res<-results%>%filter(out_var==o, cl_var==cl, mod_type== "mod_tm", model_type %in% c("lags_all_controls", "lags_all_controls_big", "lags_tp_n_mix_control") )
res$mod_type<-"TM"
g1<-ggplot(res, aes(n_lags_EXTR, loocv, color=model_type))+geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
#g2<-ggplot(res, aes(n_lags_EXTR, l3ocv, color=model_type))+geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
g3<-ggplot(res, aes(n_lags_EXTR, wr2, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
g4<-ggplot(res, aes(n_lags_EXTR, aic, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
g5<-ggplot(res, aes(n_lags_EXTR, bic, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
...
cl="TVAR"
res<-results%>%filter(out_var==o, cl_var==cl, mod_type== "mod_extr", model_type %in% c("lags_all_controls", "lags_all_controls_big", "lags_tp_n_mix_control") )
res$mod_type<-"TVAR"
g1<-ggplot(res, aes(n_lags_EXTR, loocv, color=model_type))+geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
#g2<-ggplot(res, aes(n_lags_EXTR, l3ocv, color=model_type))+geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
g3<-ggplot(res, aes(n_lags_EXTR, wr2, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
g4<-ggplot(res, aes(n_lags_EXTR, aic, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
g5<-ggplot(res, aes(n_lags_EXTR, bic, color=model_type))  +geom_point(aes(shape=mod_type), size=2)+theme_bw()+labs(shape="moderating variable", color="model")+labs( x = "lags number")+ scale_x_continuous(breaks=c(1:8))+theme(text = element_text(family = "CM Roman",size = 18))+theme(text = element_text(family = "CM Roman",size = 18))
...
### tvar - mean tvar, wd - mean wd, pext - mean tm, rx - mean tm
o<-"gr_eys"
ovar<-"EYS"
