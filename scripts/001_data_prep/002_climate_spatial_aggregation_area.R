...
### country and regional aggregation (gdl)
...
countries_borders<-giscoR::gisco_get_countries(spatialtype = "BN")
countries_borders<-countries_borders[,c(11)]
countries_borders<-sf::st_as_sf(countries_borders)
# fix crs
countries_borders<-st_transform(countries_borders, crs = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs+ towgs84=0,0,0")

for (v in file_var_names){
  for (y in years){
    plot(all_rast[[paste0(v)]][[paste0(y)]], main=paste0(v, "_", y))
    par(new=TRUE)
...
### save all dataframes 
write.csv(data_country, file = file.path(data_dir, "data_climate_country_1950_2023.csv"), row.names = FALSE)
write.csv(data_gdl, file = file.path(data_dir, "data_climate_gdl_1950_2023.csv"), row.names = FALSE)
...
data_climate_country_1950_2023 <- read_csv("data/climate_data/era5/data_climate_country_1950_2023.csv")
...
19018-15392
74*(257-208) #ok
write.csv(data_climate_country_1950_2023, file = file.path(data_dir, "data_climate_country_1950_2023_updated.csv"), row.names = FALSE)




## check missing places
data_climate_gdl_1950_2023 <- read_csv("data/climate_data/era5/data_climate_gdl_1950_2023.csv")
...
write.csv(data_climate_gdl_1950_2023, file = file.path(data_dir, "data_climate_gdl_1950_2023_updated.csv"), row.names = FALSE)
