...
g<-ggplot( data=glob_data, aes(x = year ) ) +
  ...
  aes(colour = ssp, show_guide = FALSE) +
  theme_bw()+theme(text = element_text(family = "CM Roman",size = 20))
# x11()
# print(g)
...
final<-annotate_figure(final, left = text_grob("Total Percentage Loss", rot = 90, vjust = 1, size = 20),
                bottom = text_grob("SSP Scenario", size = 20))
ggsave(filename=file.path(out_dir, paste0("sign_perc_dam_share_ssp_comparison_2050_2100_",spec,'_',vars, ".png")), final, width=11, height=4)
...
# past and proj baseline ssps (find covid loss)
