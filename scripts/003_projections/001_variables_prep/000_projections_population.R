...
## Sub-national shape file DOSE
dose_nongadm_shape_file <- sf::st_read("data/DOSE_shapefiles.gpkg")
sf::st_layers("data/gadm36_levels_gpkg/gadm36_levels.gpkg")
gadm1<- sf::st_read("data/gadm36_levels_gpkg/gadm36_levels.gpkg", layer="level1")
# unite dose new and all other gadm1  -> keep the ones manually created wrt gadm1 if there is sovrapposition
ids_both<-intersect(dose_nongadm_shape_file$GID_1, gadm1$GID_1 )
gadm1<-gadm1[-which(gadm1$GID_1 %in% ids_both),]
intersect(dose_nongadm_shape_file$GID_1, gadm1$GID_1 ) # ok
dose_shape_file<-unique(rbind(dose_nongadm_shape_file, gadm1))
# fix crs
dose_shape_file<-sf::st_as_sf(dose_shape_file)
#dose_shape_file<-sf::st_transform(dose_shape_file, crs = 4326)
dose_sub_nations<-dose_shape_file[, c(1,3,11)] 
colnames(dose_sub_nations)[which(colnames(dose_sub_nations)=="GID_0")]<-"iso3"
...
for (n in 1:length(ssp_names)){
  ...
  for (year in years){
    ...
    # extraction dose
    agg_subnat = exactextractr::exact_extract(r, dose_sub_nations, fun = "sum")
    agg_subnat<-data.frame(agg_subnat)
    agg_subnat$GID_1<-dose_sub_nations$GID_1
    agg_subnat$ssp<-ssp_names[n]
    colnames(agg_subnat)<-c(  "value", "GID_1","ssp" )
    agg_subnat$year<-year
    
    data_ssp_dose<-rbind(data_ssp_dose, agg_subnat)
...
write.csv(data_ssp_dose, file = file.path(data_dir,paste0(  "pop_projection_dose_sum_2015_2100.csv")), row.names = FALSE)
