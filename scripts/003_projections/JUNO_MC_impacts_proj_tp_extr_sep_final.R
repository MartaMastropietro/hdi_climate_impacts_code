...
coefs_dir<-"output/models/final_lag_models/conservative_N_lags_mix"
...
# select coefficients to use (tp or other variables)
pattern <- "^(diff_TM|lag_\\d+_diff_TM|TM_mean_i_diff_TM|lag_\\d+_TM_mean_i_diff_TM|diff_RR|lag_\\d+_diff_RR|RR_mean_i_diff_RR|lag_\\d+_RR_mean_i_diff_RR)$"
coefs_boot_tp<- coefs_boot[grep(pattern, rownames(coefs_boot)), ]
rownames(coefs_boot_tp)
coefs_boot_extr<- coefs_boot[-grep(pattern, rownames(coefs_boot)), ]
rownames(coefs_boot_extr)

if(vars=="tp"){coefs_boot_red<-coefs_boot_tp}
if(vars=="extr"){coefs_boot_red<-coefs_boot_extr}

cc_proj_values<- foreach(n =(1:nboot) , .combine = 'cbind', .export = c("clim_deltas_calc"), .packages = c("dplyr") )%dopar%{
  
  coefs_boot_n<-coefs_boot_red[,n]
...
arrow::write_feather(data_all,file.path(out_dir_lag,
                                        paste0("gr_",out_var,'_',spec,'_',type,'_',  agg, "_nlags",N,"_",vars, 
