...
for (y in c(2030, 2050,2080, 2100)){
  for (sc in unique(data$ssp)){
    ...
    g<-ggplot(long_data, aes(x = variable, y = 100*mean, fill = model)) +
      ...
      theme(axis.text.x = element_text(angle = 90, hjust = 1), text = element_text(family = "CM Roman",size = 20))
...
sel_countries<-c("USA",  "RUS", "CHN",  "ETH",  "AFG", "IND")
...
#        scale_x_discrete(limits = variable_order) + 
#        facet_wrap(~iso3, scales = "fixed", ncol=3) +  # Facet by country
#        labs(x = "Variable", y = "Index Change (in % points)",
#             fill = "Variable") +
#        theme_bw() +
#        scale_fill_brewer(palette = "Set3") +
#        theme(axis.text.x = element_text(angle = 60, hjust = 1),text = element_text(family = "CM Roman",size = 20))
#      
#      
#      # Save the plot
...
g<-ggplot(data_comp, aes(x = 100*delta_lifex_index_mean, y = 100*delta_income_index_mean, color = 100*delta_edu_index_mean)) +
  ...
  facet_wrap(~iso3, scales = "fixed", ncol=3) +  # Facet by iso3, allowing different y scales
  theme_bw() + 
  labs(
       x = "Lifex Index change",
       y = "Income Index change",
       color = "Edu Index \n change")+
  theme(text = element_text(family = "CM Roman",size = 20))
